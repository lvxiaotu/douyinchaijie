...
from integrations.douyin_provider import get_douyin_provider, get_douyin_provider_status, get_provider_metrics
...
router = APIRouter(prefix="/api/integrations/douyin-provider", tags=["douyin-provider"])
...
class WorkDetailRequest(BaseModel):
    ...
    region: str = Field(default="US", description="TikHub-compatible region field")
...
def integration_error(exc: Exception) -> HTTPException:
    if isinstance(exc, TikhubApiError):
        logger.warning("Douyin provider legacy TikHub failed: %s", exc)
        return HTTPException(
            status_code=exc.status_code or 502,
            detail={
                "error_type": type(exc).__name__,
                "message": str(exc),
                "hint": "Check TIKHUB_API_KEY, TIKHUB_API_BASE, TikHub quota, or request parameters.",
                "upstream": exc.to_dict(),
            },
        )
    if isinstance(exc, DouyinSpiderApiError):
        logger.warning("Douyin provider Spider failed: %s", exc)
        return HTTPException(
            status_code=502,
            detail={
                "error_type": type(exc).__name__,
                "message": str(exc),
                "hint": "Check DOUYIN_SPIDER_VENDOR_PATH, npm dependencies, DY_COOKIES, and request parameters.",
                "upstream": exc.to_dict(),
            },
        )
    if _is_upstream_unavailable(exc):
        logger.warning("Douyin provider upstream unavailable: %s", exc)
        return HTTPException(
            status_code=503,
            detail={
                "error_type": type(exc).__name__,
                "message": "Douyin provider upstream is unavailable or timed out.",
                "hint": "Check provider mode, network, cookie validity, and upstream service status.",
            },
        )
    logger.exception("Douyin provider failed")
    return HTTPException(
        status_code=500,
        detail={
            "error_type": type(exc).__name__,
            "message": str(exc),
            "hint": "Check DOUYIN_PROVIDER_MODE and the active provider configuration.",
        },
    )
...
def status() -> dict[str, Any]:
    return get_douyin_provider_status()
...
def user_videos(payload: UserVideosRequest) -> dict[str, Any]:
    try:
        ...
        return provider.get_user_videos(
...
def work_detail(payload: WorkDetailRequest) -> dict[str, Any]:
    try:
        return get_douyin_provider().get_work_detail(payload.work_url, region=payload.region)
    except Exception as exc:
        raise integration_error(exc) from exc
...
def one_video(payload: OneVideoRequest) -> dict[str, Any]:
    try:
        return get_douyin_provider().get_one_video(payload.aweme_id, region=payload.region)
    except Exception as exc:
        raise integration_error(exc) from exc
...
def video_comments(payload: VideoCommentsRequest) -> dict[str, Any]:
    try:
        return get_douyin_provider().get_video_comments(
...
def video_comment_replies(payload: VideoCommentRepliesRequest) -> dict[str, Any]:
    try:
        return get_douyin_provider().get_video_comment_replies(
...
def favorite_items(payload: FavoriteItemsRequest) -> dict[str, Any]:
    try:
        return get_douyin_provider().get_favorite_videos(
            max_items=payload.max_items,
            page_size=payload.page_size,
            max_cursor=payload.max_cursor,
        )
...
def download_favorites(payload: FavoriteDownloadRequest) -> dict[str, Any]:
    try:
        return get_douyin_provider().download_favorite_videos(
            max_items=payload.max_items,
            page_size=payload.page_size,
        )
    except Exception as exc:
